...
class PlaceholderToolbar(CMSToolbar):
    """
    Adds placeholder edit buttons if placeholders or static placeholders are
    ...
    def add_structure_mode(self):
        ...
        switcher.add_button(
            _("Content"),
            '?edit',
            active=not self.toolbar.build_mode,
            disabled=self.toolbar.build_mode
        )
        switcher.add_button(
            _("Structure"),
            '?build',
            active=self.toolbar.build_mode,
...
class BasicToolbar(CMSToolbar):
    ...
    def add_admin_menu(self):
        ...
        auth_installed = User in admin.site._registry
        if self.request.user.has_perm('user.change_user') and auth_installed:
        ...
        admin_menu.add_sideframe_item(
            _('User settings'),
            url=reverse('admin:cms_usersettings_change')
        )
    ...
    def add_language_menu(self):
        ...
        for language in get_language_objects(self.current_site.pk):
            ...
            language_menu.add_link_item(
                language['name'],
                url=url,
                active=self.current_lang == language['code']
            )
...
class PageToolbar(CMSToolbar):
    def populate(self):
        ...
        if get_cms_setting('PERMISSION'):
            has_global_page_change_permission = has_page_change_permission(
                self.request
            )
        else:
            has_global_page_change_permission = False
            # check if user has page edit permission
        can_change = (
            self.request.current_page and
            self.request.current_page.has_change_permission(self.request)
        )
        if has_global_page_change_permission or can_change:
    ...
    def post_template_populate(self):
        ...
        if self.page or statics:
            if self.toolbar.edit_mode:
                ...
                if (self.page and
                        not self.page.has_publish_permission(self.request)):
                    publish_permission = False

                for static_placeholder in dirty_statics:
                    if not static_placeholder.has_publish_permission(
                            self.request):
                        publish_permission = False

                classes = ["cms_btn-action", "cms_btn-publish"]

                if self.page and self.page.is_dirty(self.current_lang):
                    dirty = True
                elif len(dirty_statics) > 0:
                    dirty = True
                else:
                    dirty = False
                ...
                if dirty_statics or is_published:
                ...
                with force_language(self.current_lang):
                    publish_url = reverse(
                        ...
                        args=(pk, self.current_lang)
                    )
                if dirty_statics:
                    dirty_statics_pks = ','.join(
                        str(static.pk) for static in dirty_statics
                    )
                    publish_url += "?statics={}".format(dirty_statics_pks)
        ...
        if self.page:
            if (self.page.has_change_permission(self.request) and
                    self.page.is_published(self.current_lang)):
                self.add_draft_live()
            elif statics:
                for static_placeholder in statics:
                    if static_placeholder.has_change_permission(self.request):
    ...
    def add_draft_live(self):
        self.toolbar.add_item(
            TemplateItem(
                "cms/toolbar/items/live_draft.html",
    ...
    def change_language_menu(self):
        ...
        if len(languages) > 1 and self.current_lang and len(remove) > 1:
            ...
            for language in languages:
                if (self.current_lang == language['code'] or
                        language['code'] in add):
                    continue
                url = reverse(
                    'admin:cms_page_copy_language',
                    args=[self.page.pk]
                )
                question = _('Are you sure you want copy all plugins '
                             'from %s?') % language['name']
                language_menu.add_ajax_item(
                    _("Copy all plugins from %s") % language['name'],
                    action=url,
                    data={
    ...
    def add_page_menu(self):
        ...
        current_page_menu.add_link_item(
            _('Edit this Page'),
            disabled=self.toolbar.edit_mode,
            url='?edit'
        )
        ...
        if self.toolbar.build_mode or self.toolbar.edit_mode:
            ...
            action = reverse(
                'admin:cms_page_change_template',
                args=(self.page.pk,)
            )
        ...
        if get_cms_setting('PERMISSION'):
            permissions_url = reverse(
                'admin:cms_page_permissions',
                args=(self.page.pk,)
            )
            permission_disabled = (
                not self.toolbar.edit_mode or
                not self.page.has_change_permissions_permission(self.request)
            )
            current_page_menu.add_modal_item(
                _('Permissions'),
                url=permissions_url,
        ...
        nav_action = reverse(
            'admin:cms_page_change_innavigation',
            args=(self.page.pk,)
        )
        ...
        with force_language(self.current_lang):
            # We use force_language because it makes no sense to redirect a
            # user who just deleted a german page to an english page (user's
            # default language) simply because the url
            # /en/some-german-page-slug will show nothing
            if self.page.parent:
                # If this page has a parent, then redirect to it
                on_delete_redirect_url = self.page.parent.get_absolute_url(
                    language=self.current_lang
                )
            else:
                # If there's no parent, we redirect to the root.
                # Can't call Page.objects.get_home() because the user could
                # very well delete the homepage causing get_home to throw an
                # error.
                # Let's keep in mind that if the user has deleted the last
                # page, and django is running on DEBUG == False this redirect
                # will cause a 404...
    ...
    def add_history_menu(self):
        ...
        if 'reversion' in settings.INSTALLED_APPS:
            ...
            if self.page.revision_id:
                current_revision = Revision.objects.get(
                    pk=self.page.revision_id
                )
                has_undo = versions.filter(
                    revision__pk__lt=current_revision.pk
                ).count() > 0
                has_redo = versions.filter(
                    revision__pk__gt=current_revision.pk
                ).count() > 0
            else:
                has_redo = False
        ...
        revert_action = reverse(
            'admin:cms_page_revert_page',
            args=(self.page.pk, self.current_lang)
        )
        ...
        history_menu.add_modal_item(
            _('View history'),
            url=reverse('admin:cms_page_history', args=(self.page.pk,))
        )
