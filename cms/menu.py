...
def get_visible_pages(request, pages, site=None):
    ...
    page_permissions = PagePermission.objects.filter(
        can_view=True
    ).select_related('page', 'group__users')
    ...
    for perm in page_permissions:
        ...
        if perm is not None and perm not in restricted_pages[perm.page.pk]:
            ...
            if perm.grant_on in [ACCESS_PAGE, ACCESS_PAGE_AND_CHILDREN,
            ...
            if perm.grant_on in [ACCESS_CHILDREN, ACCESS_PAGE_AND_CHILDREN]:
                ...
                for pk, public_id in child_ids:
                    restricted_pages[pk].append(perm)
                    restricted_pages[public_id].append(perm)
            # add descendants
            elif perm.grant_on in [ACCESS_DESCENDANTS,
                                   ACCESS_PAGE_AND_DESCENDANTS]:
    ...
    def has_global_perm():
        if has_global_perm.cache < 0:
            has_global_perm.cache = (
                1 if request.user.has_perm('cms.view_page') else 0
            )
...
def page_to_node(page, home, cut):
    ...
    if page.navigation_extenders:
        # Is this page an apphook? If so, we need to handle the apphooks's
        # nodes
        extenders.append(page.navigation_extenders)
    lang = get_language()
    # Only run this if we have a translation in the requested language for this
    # object. The title cache should have been prepopulated in
    # CMSMenu.get_nodes but otherwise, just request the title normally
...
class CMSMenu(Menu):
    def get_nodes(self, request):
        ...
        for page in pages:
            # Pages are ordered by tree_id, therefore the first page is the
            # root of the page tree (a.k.a "home")
            ...
            if (is_home or page.parent_id in home_children) and home_cut:
                home_children.append(page.pk)
            if (is_home and home.in_navigation) or not is_home:
                first = False
            ids[page.id] = page
        ...
        titles = list(
            get_title_queryset(request).filter(
...
class NavExtender(Modifier):
    def modify(self, request, nodes, namespace, root_id, post_cut, breadcrumb):
                ...
                for ext in extenders:
                    ...
                    for extnode in nodes:
                        ...
                        if extnode.namespace == ext and not extnode.parent_id:
                            if (node.attr.get("is_home", False) and
                                    not node.visible):
        ...
        for menu in menu_pool.menus.items():
            if (hasattr(menu[1], 'cms_enabled') and
                    menu[1].cms_enabled and
                    not menu[0] in exts):
                for node in nodes:
...
class SoftRootCutter(Modifier):
    ...
    Softroot description from the docs:
        ...
        For example, you’re on the page -Introduction to Bleeding-?, so the
